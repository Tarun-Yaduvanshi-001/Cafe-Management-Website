...
export const signup = async (req, res) => {
  try {
    ...
    const newUser = await User.create({ email, name, phone, password: hashPassword });
...
export const login = async (req, res) => {
  try {
    ...
    const token = jwt.sign(tokenPayload, config.auth.JWT_SECRET, { expiresIn: '1d' });
    ...
    return res.status(200).json({
      ...
      name: user.name,
      role: user.role,
      loyaltyPoints: user.loyaltyPoints, // FIX: Add loyalty points to the response
    });
  } catch (error) {
...
export const verifyUser = async (req, res) => {
  try {
    ...
    // Also fetch the user from DB to get the latest loyalty points
